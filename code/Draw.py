...
class CompareIFPlayer():
    ...
    def draw_all(self):
        ...
        plt.bar(index, self.p1_calculate_list, color="steelblue", width=0.35)
        plt.bar(index+0.35, self.p2_calculate_list, color="darkred", width=0.35)
        plt.xticks(index, x, fontsize=15)

        plt.subplot(212)
        plt.title(f"마구마구 능력치 : {self.p1_name} vs {self.p2_name}" , fontsize=15)
        plt.bar(index, ma9_1Y, color="steelblue", width=0.35)
        plt.bar(index+0.35, ma9_2Y, color="darkred", width=0.35)
        plt.xticks(index, x, fontsize=15)
